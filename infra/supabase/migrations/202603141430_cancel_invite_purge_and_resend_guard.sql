  ...
  if not v_has_completed
    ...
    select exists(
      ...
      where m.user_id = any(v_user_ids)
        and lower(coalesce(t.status, '')) in ('active', 'completed', 'activated')
    )
    into v_has_completed;
  end if;
...
create or replace function public.is_onboarding_invite_resend_ready(p_email text)
returns table(
  ready boolean,
  reason text
)
...
begin
  ...
  into v_exists;
  if v_exists then
    return query select false, 'auth.users row still exists for this email.';
    return;
  end if;
  ...
  if to_regclass('public.pending_onboarding') is not null then
    ...
    if v_exists then
      return query select false, 'pending_onboarding row still exists for this email.';
      return;
    end if;
  end if;
  ...
  if to_regclass('public.profiles') is not null then
    execute $sql$
      select exists(
        select 1
        from public.profiles p
        where lower(coalesce(to_jsonb(p) ->> 'email', '')) = $1
      )
    $sql$
    into v_exists
    using v_email;
    ...
    if v_exists then
      return query select false, 'profiles row still exists for this email.';
      return;
    end if;
  end if;
  ...
  if to_regclass('public.users') is not null then
    execute $sql$
      select exists(
        select 1
        from public.users u
        where lower(coalesce(to_jsonb(u) ->> 'email', '')) = $1
      )
    $sql$
    into v_exists
    using v_email;
    ...
    if v_exists then
      return query select false, 'users row still exists for this email.';
      return;
    end if;
  end if;

  return query select true, 'ready';
end;
$$;

revoke all on function public.cancel_onboarding_invite_and_purge(text) from public, anon, authenticated;
revoke all on function public.is_onboarding_invite_resend_ready(text) from public, anon, authenticated;

grant execute on function public.cancel_onboarding_invite_and_purge(text) to service_role;
grant execute on function public.is_onboarding_invite_resend_ready(text) to service_role;
