...
async def stripe_webhook(
    ...
    if event_type == "checkout.session.completed":
        ...
        if subscription_id:
            _update_org_status(db, billing, str(subscription_id), None, None, None)
            return {"received": True}

        if checkout_data.get("tenant_id"):
            logger.warning(
                "Stripe checkout session %s for tenant %s had no subscription id",
                checkout_data.get("reference"),
                checkout_data.get("tenant_id"),
            )
            return {"received": True}

        # Legacy checkout links created before tenant metadata existed can still
...
def _update_org_status(
    ...
    org = None
    tenant_id_raw = subscription_metadata.get("tenant_id") or subscription_metadata.get("organization_id")
    if tenant_id_raw:
        try:
            org = db.get(Organization, uuid.UUID(str(tenant_id_raw)))
        except (TypeError, ValueError):
            logger.warning("Subscription %s has invalid tenant_id metadata %s", subscription_id, tenant_id_raw)
    ...
    if not org:
        # Legacy fallback for old Checkout links without tenant metadata.
    ...
    db.add(org)
    _sync_tenant_subscription_state(db, tenant_id=org.id, subscription_status=subscription_status, org_active=org_active)
...
def _sync_tenant_subscription_state(
    ...
    *,
    ...
    db.execute(
        ...
        {"status": tenant_status, "tenant_id": str(tenant_id)},
    )
