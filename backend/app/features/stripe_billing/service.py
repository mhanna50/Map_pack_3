...
class BillingService:
    ...
    def create_checkout_session(
        ...
        *,
        ...
        success_url = self._resolve_checkout_redirect_url(
            ...
            fallback_path="/payments/success",
            override_path=success_path,
        )
        cancel_url = self._resolve_checkout_redirect_url(
            configured_url=str(settings.STRIPE_CANCEL_URL) if settings.STRIPE_CANCEL_URL else None,
            fallback_path="/payments/cancel",
            override_path=cancel_path,
        )
    ...
    def _resolve_checkout_redirect_url(
        *,
        ...
        if override_path:
            return BillingService._client_url_for_path(override_path)
        if configured_url and not BillingService._is_placeholder_url(configured_url):
            return configured_url
        return BillingService._client_url_for_path(fallback_path)

    @staticmethod
    def _client_url_for_path(path: str) -> str:
        parts = urlsplit(path)
        if parts.scheme or parts.netloc:
            raise ValueError("Checkout redirect paths must be relative")
        normalized_path = path if path.startswith("/") else f"/{path}"
        return f"{str(settings.CLIENT_APP_URL).rstrip('/')}{normalized_path}"

    @staticmethod
    def _is_placeholder_url(url: str) -> bool:
        parts = urlsplit(url)
        return parts.netloc.lower() in {"yourapp.com", "www.yourapp.com"}
