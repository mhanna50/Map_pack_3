...
export function PageHero({
  ...
  return (
    <section className={cn("relative overflow-hidden px-6 pb-16 pt-32 md:pb-20", className)}>
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(180deg,#fbf4e8_0%,#F8F3EA_100%)]" />
      <div className="absolute inset-x-0 bottom-0 -z-10 h-px bg-[#D8CFC1]" />
      <div
        className={cn(
      ...
      >
        <div className={cn("max-w-3xl space-y-5", !children && "mx-auto text-center")}>
          ...
          <h1 className="text-balance text-4xl font-semibold leading-[1.04] text-[#14213D] md:text-6xl">
            {title}
          </h1>
          <p className="text-pretty text-lg leading-8 text-[#5F6673] md:text-xl">{description}</p>
        </div>
